...
| Config/biến môi trường | python-dotenv + Pydantic Settings — class Pydantic Settings ĐẦU TIÊN của dự án (`src/services/config.py`, chỉ `SENTRY_DSN`/`APP_ENVIRONMENT`) tạo ở bước 6.3; cấu hình MySQL vẫn đọc `os.environ` trực tiếp trong `db_connection.py`, CHƯA refactor sang class này (ngoài phạm vi 6.3, xem HD-20) | Tuần 2 (python-dotenv) / Tuần 6 bước 6.3 (Pydantic Settings) |
...
*(Migration và import đã chạy thật từ Tuần 2 — xem HD-09, HD-10. Lệnh app đã có code thật từ bước 3.1 (Auth & RBAC) — xem HD-11. Lệnh test ban đầu chỉ có cho `src/auth/` từ bước 3.3 (13 test, HD-13), mở rộng toàn bộ `src/services/` ở bước 7.1 (HD-21). Lệnh lint/format thêm từ bước 3.4 — xem HD-14.)*

```bash
# Chạy app local (có code thật từ bước 3.1 — trang demo đăng nhập + hiển thị phạm vi RBAC, xem HD-11)
streamlit run src/app/main.py

# Chạy test (175 test cho toàn bộ src/services/ + src/auth/, 88% coverage, chạy trên SQLite
# in-memory — không đụng MySQL "lacco" thật. Ban đầu chỉ 13 test cho src/auth/ ở HD-13 (Tuần 3),
# mở rộng phủ hết src/services/ + src/auth/admin_actions.py ở bước 7.1, xem HD-21)
...
- **Rủi ro bảo mật nghiêm trọng nhất của dự án** (đã ghi trong Kế hoạch triển khai, mục 7): Streamlit chia sẻ state ở cấp module giữa các phiên người dùng. Nếu `cache_data`/`cache_resource` không gắn tham số theo user, dữ liệu tài chính của user A có thể lộ sang user B.
  ...
  - **Gotcha đã gặp thật (bước 7.1, xem commit `be4b417`):** `@st.cache_data` dùng `pickle` để lưu/đọc GIÁ TRỊ TRẢ VỀ, không chỉ hash tham số đầu vào. Nếu hàm cache trả về instance của 1 class/enum tự định nghĩa trong `src/` (ví dụ `DataScope`, `UserRole`), file-watcher dev-mode của Streamlit reload lại module nguồn khi phát hiện file `src/` đổi trên đĩa giữa các lần rerun — tạo 2 class cùng tên nhưng khác identity, `pickle` từ chối ghi cache (`PicklingError: Can't pickle <enum ...>: it's not the same object as ...`), lỗi hiện không cố định (chạy được vài lần, fail vài lần). **Không sửa được bằng `hash_funcs`** (chỉ ảnh hưởng cách hash tham số ĐẦU VÀO, không ảnh hưởng serialize giá trị TRẢ VỀ). Cách sửa đúng: hàm được `@st.cache_data` bọc chỉ trả về kiểu builtin thuần (`dict`/`str`/`int`/`bool`/`None`/`tuple`), convert sang/từ dataclass thật ở ngay bên ngoài hàm cache (xem `_scope_to_plain_dict()`/`_plain_dict_to_scope()` trong `src/app/main.py`). Áp dụng cho MỌI hàm `@st.cache_data`/`@st.cache_resource` sau này có trả về dataclass/enum tự định nghĩa, không chỉ `_cached_data_scope()`.
...
- **v9 (11/09/2026, chốt Tuần 6 phần 6.1/6.2):** Cập nhật sau khi hoàn thành bước 6.1 xuất Excel/PDF — HD-18, bước 6.2 test tải — HD-19. 3 thay đổi: (1) mục 2 — thêm nguyên tắc graceful fallback cho tính năng phụ thuộc hạ tầng không ổn định trên mọi môi trường, rút ra từ sự cố `kaleido`/Chrome headless treo ở bước 6.1; (2) mục 3 — cập nhật bảng tech stack: chốt ReportLab (không phải WeasyPrint) cho Export PDF và openpyxl cho Export Excel, thêm dòng `kaleido` kèm rủi ro môi trường Windows Server chưa xác nhận (mang sang bước 7.2); (3) mục 7 — thêm ngưỡng hiệu năng định lượng đã chốt với COO ở bước 6.2 (p95<3 giây/hàm, 0% lỗi), bổ sung cho tiêu chí định tính sẵn có ở Kế hoạch triển khai mục 8. **Không sửa** `.claude/rules/trang-thai-yeu-cau.md` hay `.claude/agents/report-builder-agent.md` ở bản này — rà soát cho thấy 6.1/6.2 không phát sinh quyết định công thức/RBAC báo cáo mới (2 việc chính thuộc hạ tầng export/kiểm thử, không phải xây module báo cáo) và không có dòng "Cần làm rõ" nào chuyển trạng thái. Bài học: không phải bước nào cũng cần đồng bộ cả 3 file luật (CLAUDE.md + 2 file `.claude/`) như Tuần 4/5 — điều quan trọng là tự hỏi đúng câu "quyết định bước này thuộc phạm vi file nào", thay vì mặc định sửa cả 3 hoặc mặc định không sửa file nào. **Lưu ý:** tại thời điểm soạn v9, Tuần 6 còn 2 bước chưa xong (6.3 Sentry, 6.4 Review & nhật ký) nên chưa phải bản "chốt tuần" đầy đủ — xem v10.
- **v12 (23/09/2026, rà soát trước bước 7.2):** Phát hiện khi rà soát trước khi bắt đầu bước 7.2 (Deploy Windows Server) — mục 5 vẫn ghi lệnh `pytest` cũ từ v5 ("13 test cho `src/auth/`"), lỗi thời so với thực tế sau bước 7.1 (175 test, phủ cả `src/services/`). 1 thay đổi: mục 5 — cập nhật lệnh `pytest` đúng phạm vi hiện tại (`--cov=src` thay vì `--cov=src/auth`), ghi rõ mốc mở rộng (HD-13 → HD-21). Không sửa file `.claude/` nào — đây thuần là sửa lệnh tham khảo, không phải quyết định nghiệp vụ/RBAC mới. Bài học: rà soát "CLAUDE.md có phần nào lỗi thời" nên làm TRƯỚC mỗi bước lớn tiếp theo (không chỉ lúc chốt tuần) — đây là staleness tồn tại từ bước 7.1 nhưng chỉ phát hiện khi chuẩn bị sang 7.2, suýt mang theo sang các tuần sau.
