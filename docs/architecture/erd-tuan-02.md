...
4. **Giả định mã khoá dùng chung giữa FT và AMIS** — các bảng nguồn AMIS (`cost`, `budget`, `personnel_cost`, `debt`) tham chiếu FK sang các bảng danh mục vốn xây từ dữ liệu FT (`division`, `department`, `employee`, `customer`). Thiết kế hiện tại giả định 2 hệ thống dùng chung mã Khối/Phòng/Nhân viên/Khách hàng khi import Excel/CSV. **Cần xác nhận:** AMIS và FT có cùng bộ mã này không, hay cần bảng mapping trung gian (VD: `amis_code` ↔ `ft_code`) ở bước 2.2? Đây là rủi ro cao nhất nếu sai, vì ảnh hưởng toàn bộ nhóm báo cáo Chi phí và Công nợ.
